...
def background():
    ...
    arena_config={0: ("Water Pitcher",15 ,"1-1"), 1: ("Pebble1", 18, "3-3"), 4: ("Pebble2",9, "2-2"),
                    5: ("Pebble3",6,"1-1")}
    Robot_start ="START - 2"
    if (Robot_start == "START - 1"):
        START = 16
    ...
    in it. Everything worked fine if we used that exact graph as a reference. But a coue/] e of days before the 
...
def init_gl():
    global texture_object, texture_background, text
    ...
    load('crowInitBom', frames)  # Crow animation flying to the pebble

    load('crowwwbendd', frames_bend)  # Crow animation with bent neck

    load('crowPebbleBom', frames_crowrock)  # Crow animation flying the pebble back to the water pitcher

    load('asssooo', aso)

  # Crow animation flying the pebble back to the water pitcher
...
def overlay(img, ar_list, ar_id, texture_file):
    ...
    view_matrix = np.array([[rmtx[0][0], rmtx[0][1], rmtx[0][2], centre[0]*20/640-10],
                            [rmtx[1][0], rmtx[1][1], rmtx[1][2], centre[1]*15/480 - 15/2],
                            [rmtx[2][0], rmtx[2][1], rmtx[2][2], 17],
    ...
    if str(ar_id) == '10':  # AR ID for crow is always 10
        ...
        if (bend == 1):
            ...
            view_matrix = np.array([[0.96999244, 0.23894275, -0.04495581, 0],
                                    [0.05283473, -0.02666864, 0.9982471, 4],
                                    [0.23732499, -0.97066737, -0.03849286, 17],
            ...
            print(rmtx)
            glLoadMatrixd(view_matrix)
            glCallList(text.gl_list)
            glPopMatrix()

        elif (flaga == 'p' and flag == 0) or (flagb == 'p' and flag == 'd') or (flagc == 'p' and flag == 'c'):
            glCallList(next_frame(frames_crowrock))
        elif (flag == 'd' and flagb == 0) or (flag == 'c' and flagc == 0):
            glScale(2,2,2)
            glCallList(next_frame(aso))
        elif (flag == 0 and flaga == 0):
