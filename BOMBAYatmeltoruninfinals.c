...
int main(void)
{
    ...
    while (1) 
    {
			...
			if ((Center_white_line>100 && Right_white_line>100 && Left_white_line>100) || (Center_white_line + Left_white_line>250 ) || (Center_white_line + Right_white_line> 250)){
				//flag = 7;
				while(!(UCSR0A & TE));			//waiting to transmit
				UDR0 = ins[j];
				...
				flag2=1;
					...
					forward();
					/*stop();*/
					servoaction(7);				//arm is taken upwards to avoid collisions
					...
					magnet_off();
					buzzer_on();
					_delay_ms(100);					//buzzer is turned on for five seconds
