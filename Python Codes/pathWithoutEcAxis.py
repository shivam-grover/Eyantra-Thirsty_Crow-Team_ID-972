...
def axisToIns(axisIns, axisW, startAxis):
    ...
    if (axisIns[len(axisIns) - 1] == axisPathWaterpi[0]):
        ins.append('a')  # 180 degree direction change
    ...
    for i in range(1, len(axisPathWaterpi)):
        ...
        #
        if (axisIns[i] == 2 and axisPathWaterpi[i] == 2):
            ins.append('e')  # the bot and the pebble are aligned across the same axis so the bot just has to move forward (no turns)
        if (axisIns[i] == 1 and axisPathWaterpi[i] == 1):
            ins.append('e')  # the bot and the pebble are aligned across the same axis so the bot just has to move forward (no turns)
        if (axisIns[i] == 3 and axisPathWaterpi[i] == 3):
            ins.append('e')  # the bot and the pebble are aligned across the same axis so the bot just has to move forward (no turns)

        # for the cases when the bot is not aligned with the pebble axis

        if (axisIns[i] == 1 and axisPathWaterpi[i] == 2):
            ins.append('g')  # turn Right by 60 degrees
        if (axisIns[i] == 1 and axisPathWaterpi[i] == 3):
            ins.append('h')  # turn Right by 30 degrees
        if (axisIns[i]==2 and axisPathWaterpi[i] == 1):
            ins.append('i')  # turn left by 60 degrees
        if (axisIns[i] == 2 and axisPathWaterpi[i] == 3):
            ins.append('j')  # turn Right by 60 degrees
        if (axisIns[i] == 3 and axisPathWaterpi[i] == 1):
            ins.append('k')  # turn Right by 30 degrees
        if (axisIns[i]==3 and axisPathWaterpi[i] == 2):
            ins.append('m')  # turn left by 60 degrees
